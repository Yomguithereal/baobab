  ...
  Baobab.prototype.update = function update(path, operation) {
    ...
    if (/[M|m]erge/.test(operation.type)) {
      ...
      if (_type2['default'].object(monkeysNode)) {
        ...
        if (/deep/.test(realOperation.type)) realOperation.value = deepMerge({}, deepClone(monkeysNode), realOperation.value);else realOperation.value = shallowMerge(deepMerge({}, getIn(this._data, solvedPath).data, monkeysNode), realOperation.value);
      }
    }
